    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Binance TR Otomatik Bağlantı</h2>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Binance TR (`binance.tr`) Doğrudan Spot Cüzdan Senkronizasyonu
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <TradingViewWidget symbol="BINANCE:BTCTRY" />
        </div>
        <div className="lg:col-span-1 p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
              Binance TR API Key & Secret bilgilerinizi girerek spot cüzdan bakiyelerinizi portföyünüze otomatik aktarabilirsiniz.
            </p>
          </div>
          <button
            onClick={() => setIsApiModalOpen(true)}
          ...
          >
            <Key className="w-4 h-4" />
            <span>Bakiyeleri Senkronize Et</span>
          </button>
        </div>
      </div>
